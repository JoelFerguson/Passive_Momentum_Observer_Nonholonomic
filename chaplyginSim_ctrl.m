...
Kd = eye(2);
% Define system input
u = @(q,xp,phi) -(sys.Gr(q)\[obs.T(q) obs.B(q).'])*Kp*q - Kd*obs.G(q).'*obs.ph(q,xp,phi);
% Define observer input
uo = @(q,xp,phi) -Kd*obs.G(q).'*obs.ph(q,xp,phi);
...
ode = @(t,x) [sys.dx(x(1:5),u(x(1:3),x(6:7),x(8)));
                obs.dxp(x(1:3),obs.ph(x(1:3),x(6:7),x(8)),u(x(1:3),x(6:7),x(8)),uo(x(1:3),x(6:7),x(8)),x(8));
                obs.dphi];
...
hold on
plot(res.t,res.q(:,1))
legend('$$\theta$$','$$x$$','Interpreter','Latex')
xlabel('time (s)')
ylabel('States')
...
%     q = (rand(3,1)-0.5)*k;
%     p = (rand(2,1)-0.5)*k;
%     sigma = max(sigma,sqrt(norm(obs.Sb(q,p)))/sqrt(norm(p)));
% end
% sigma
